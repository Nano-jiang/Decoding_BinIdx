...
ACCbin_trials = nan(trials_num, binNum_all);
ErrBin_trials = nan(trials_num, binNum_all);

for ti = 1:trials_num
    % 1. 划分索引
    idxTest = ti;
    idxTrain = setdiff(trials_id, idxTest);
    ...
    XTrain = reshape(tempTrain, [binNum_all * length(idxTrain), n]);
    YTrain = repmat((bin_idx_all)', length(idxTrain), 1);

    rowsToDrop = any(isnan(XTrain), 2); % 只要row中出现NaN，就排除
    cleanXTrain = XTrain(~rowsToDrop, :);
    cleanYTrain = YTrain(~rowsToDrop);

    % 3. 准备测试数据：即当前的这个 trial
    % 结果维度：binNum x n 
    XTest = NeuronDataBinAll(:,:,idxTest)'; 
    rowsToDrop = any(isnan(XTest), 2);
    cleanXTest = XTest(~rowsToDrop, :);
    
    % 4. 训练与预测
    mdl = fitcecoc(cleanXTrain, cleanYTrain);
    YHat = nan(binNum_all,1); % YHat是一个可能有NaN的值，NaN出现在测试集中bin上无数据的时候
    YHat(~rowsToDrop) = predict(mdl, cleanXTest);
    
    % 5. 记录当前 trial 的准确率 (每个 bin 预测对了吗？)
    acc_temp = double(YHat' == bin_idx_all); 
    acc_temp(isnan(YHat')) = NaN;  
    ACCbin_trials(ti, :) = acc_temp; 

    % 6. 记录解码误差
    for bi = 1:binNum_all
        pred_bin = YHat(bi);
        true_bin = bin_idx_all(bi);
        
        if ~isnan(pred_bin)    
            if (abs(pred_bin - true_bin) <= 4) || ...
                   (pred_bin <= 10 && true_bin <= 10) % pred和true在同一条臂 或 分别在中央臂和左臂上
                error_bin = abs(pred_bin - true_bin);
            elseif (pred_bin > 10 && true_bin <= 5) || ...
                   (pred_bin <= 5 && true_bin > 10) % pred和true分别在中央臂和右臂上
                error_bin = abs(pred_bin - true_bin) - binNum(1);
            elseif (pred_bin > 10 && (true_bin > 5 && true_bin <= 10)) || ...
                   ((pred_bin > 5 && pred_bin <= 10) && true_bin > 10) % pred和true分别在左右臂上
                error_bin = pred_bin + true_bin - (6+11) + 1; % 6 和 11 分别是左右臂首个 bin
            end
            ErrBin_trials(ti, bi) = error_bin * bin_size;
        else
            ErrBin_trials(ti, bi) = NaN;
        end
    end
end

% 得到每个 Bin 的平均准确率 (1 x binNum)
ACCbin_Avg = mean(ACCbin_trials, 1, 'omitnan');
ACC_Avg = mean(ACCbin_Avg);

ErrBin_Avg = mean(ErrBin_trials, 1, 'omitnan');
Err_Avg = mean(ErrBin_Avg);         

% --- 计算 Shuffle 解码准确率 ---
sft = 100;
rng("shuffle"); 
ACCbinSf = nan(sft, binNum_all);
ErrBinSf = nan(sft, binNum_all);

for sfi = 1:sft
    fprintf('sfi = %d\n', sfi);
    ACCbinSf_trials = nan(trials_num, binNum_all);
    ErrBinSf_trials = nan(trials_num, binNum_all);
    for ti = 1:trials_num
         % 1. 划分索引
        idxTest = ti;
        idxTrain = setdiff(trials_id, idxTest);
        ...
        % 展平为: [(binNum * trainTrialNum), nNeuron]
        XTrain = reshape(tempTrain, [binNum_all * length(idxTrain), n]);
        YTrain_shf = repmat((bin_idx_all)', length(idxTrain), 1);
        YTrain_shf = YTrain_shf(randperm(length(YTrain_shf)));

        rowsToDrop = any(isnan(XTrain), 2); % 只要row中出现NaN，就排除
        cleanXTrain = XTrain(~rowsToDrop, :);
        cleanYTrain_shf = YTrain_shf(~rowsToDrop);
        ...
        % 4. 训练与测试
        mdl = fitcecoc(cleanXTrain, cleanYTrain_shf);
        YHat = nan(binNum_all,1); % YHat是一个可能有NaN的值，NaN出现在测试集中bin上无数据的时候
        YHat(~rowsToDrop) = predict(mdl, cleanXTest);
        ...
        for bi = 1:binNum_all
            ...
            if ~isnan(pred_bin)    
                ...
                ErrBinSf_trials(ti, bi) = error_bin * bin_size;
            else
                ErrBinSf_trials(ti, bi) = NaN;
            end
        end
    end
    
    % 记录这一次 shuffle 实验的平均结果
    ACCbinSf(sfi, :) = mean(ACCbinSf_trials, 1, 'omitnan');
    ErrBinSf(sfi, :) = mean(ErrBinSf_trials, 1, 'omitnan');
end

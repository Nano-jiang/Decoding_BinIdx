...
% 更新时间：2026/04/13
% 用4-fold进行模型训练(fold_num*bin_num)*n，而非直接将4-fold平均，
% 1-fold进行模型预测
% NeuronP预先zscore
...
% zscore
NeuronP1_zscored = nan(n, m);
for ni = 1:n
    current_neuron = NeuronP1(ni, :);
    
    % 找到非 NaN 的索引
    valid_idx = ~isnan(current_neuron);
    
    if any(valid_idx)
        data = current_neuron(valid_idx);
        % 计算该神经元的 Z-Score
        mu = mean(data);
        sigma = std(data);
        
        if sigma > 0
            NeuronP1_zscored(ni, valid_idx) = (data - mu) / sigma;
        else
            % 如果标准差为 0（数据全相等），设为 0 或保持 NaN
            NeuronP1_zscored(ni, valid_idx) = 0; 
        end
    end
end
...
bin_width_cm = width_cm/bin_num(1);
bin_height_cm = height_cm/bin_num(2);
...
ACCbin_kfolds = nan(kfolds, binNum);
ErrBin_kfolds = nan(kfolds, binNum);

for ki = 1:kfolds
    fmsTest = folds{ki}; 
    % 找出训练集包含哪几个 Fold 的索引
    train_fold_indices = setdiff(1:kfolds, ki); 
    num_train_folds = length(train_fold_indices);
    
    % --- 初始化数据 ---
    % 训练集改为 (binNum * num_train_folds) x n
    tempXTrain = nan(binNum * num_train_folds, n);
    % 标签也要对应重复 4 次
    YTrain = repmat((1:binNum)', num_train_folds, 1);
    
    % 测试集保持不变 (binNum x n)
    tempXTest = nan(binNum, n);
    
    % --- 填充训练集数据 ---
    for tf_idx = 1:num_train_folds
        current_fold_id = train_fold_indices(tf_idx);
        fms_current_train = folds{current_fold_id}; % 获取当前这一折的帧索引
        
        for bi = 1:binNum
            % 核心：只找当前这一折与当前 bin 的交集
            v_train = intersect(fms_bin_scope_OF{bi}, fms_current_train, 'stable');
            
            % 计算该 Fold 在 tempXTrain 中的存储行位置
            row_idx = (tf_idx - 1) * binNum + bi;
            
            if ~isempty(v_train)
                tempXTrain(row_idx, :) = mean(NeuronP1_zscored(:, v_train), 2, 'omitnan')'; 
            end
        end
    end
    
    rowsToDrop = any(isnan(tempXTrain), 2); % 只要row中出现NaN，就排除
    cleanXTrain = tempXTrain(~rowsToDrop, :);
    cleanYTrain = YTrain(~rowsToDrop);

    mdl = fitcecoc(cleanXTrain, cleanYTrain);

    % --- 填充测试集数据 (逻辑不变) ---
    for bi = 1:binNum
        v_test = intersect(fms_bin_scope_OF{bi}, fmsTest, 'stable');
        if ~isempty(v_test)
            tempXTest(bi, :) = mean(NeuronP1_zscored(:, v_test), 2, 'omitnan')'; 
        end
    end

    rowsToDrop = any(isnan(tempXTest), 2);
    cleanXTest = tempXTest(~rowsToDrop, :);
    
    YHat = nan(binNum,1); % YHat是一个可能有NaN的值，NaN出现在测试集中bin上无数据的时候
    YHat(~rowsToDrop) = predict(mdl, cleanXTest);
    
    % 保留某一折上预测值为NaN的位置，当作没有预测值
    acc_temp = double(YHat' == (1:binNum)); 
    acc_temp(isnan(YHat')) = NaN;  
    ACCbin_kfolds(ki, :) = acc_temp;

    % 计算解码误差 (物理距离)
    pred_coords = nan(binNum, 2); % 预测的 [x, y]
    valid_idx = ~isnan(YHat); 
    if any(valid_idx)
        pred_coords(valid_idx, :) = bin_centers(YHat(valid_idx), :);
    end
...
ACCbin_Avg = mean(ACCbin_kfolds, 1, 'omitnan'); 
ACC_Avg = mean(ACCbin_Avg);
ErrBin_Avg = mean(ErrBin_kfolds, 1, 'omitnan'); 
Err_Avg = mean(ErrBin_Avg);

% --- 第二部分：计算 Shuffle 解码准确率 ---
sft = 100;
rng("shuffle"); 
ACCbinSf = nan(sft, binNum);
ErrBinSf = nan(sft, binNum);

for sfi = 1:sft
    fprintf('sfi = %d\n', sfi);
    ACCbinSf_kfolds = nan(kfolds, binNum);
    ErrBinSf_kfolds = nan(kfolds, binNum);
    ...
    for ki = 1:kfolds
        ...
        % 标签也要对应重复 4 次
        YTrain = repmat((1:binNum)', num_train_folds, 1);
        YTrain_shuffled = YTrain(randperm(length(YTrain)));

        % 测试集保持不变 (binNum x n)
        tempXTest = zeros(binNum, n);
        ...
        rowsToDrop = any(isnan(tempXTrain), 2); % 只要row中出现NaN，就排除
        cleanXTrain = tempXTrain(~rowsToDrop, :);
        cleanYTrain_shuffled = YTrain_shuffled(~rowsToDrop);

        mdl = fitcecoc(cleanXTrain, cleanYTrain_shuffled);
        ...
        acc_temp = double(YHat' == (1:binNum)); 
        acc_temp(isnan(YHat')) = NaN;  
        ACCbinSf_kfolds(ki, :) = acc_temp;

        % 计算解码误差 (物理距离)
        pred_coords_sf = nan(binNum, 2); % 预测的 [x, y]
        valid_idx = ~isnan(YHat); 
        if any(valid_idx)
            pred_coords_sf(valid_idx, :) = bin_centers(YHat(valid_idx), :);
        end
        true_coords_sf = bin_centers(1:binNum, :);    % 真实的 [x, y]
        
        % 计算欧几里得距离: sqrt((x1-x2)^2 + (y1-y2)^2)
        dist_error_sf = sqrt(sum((pred_coords_sf - true_coords_sf).^2, 2));
        ErrBinSf_kfolds(ki, :) = dist_error_sf';
     end
    ACCbinSf(sfi, :) = mean(ACCbinSf_kfolds, 1, 'omitnan');
    ErrBinSf(sfi, :) = mean(ErrBinSf_kfolds, 1, 'omitnan');
end
